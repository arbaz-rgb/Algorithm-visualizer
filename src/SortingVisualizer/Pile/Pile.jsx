...
export default class Pile extends Component {
  render() {
    const {
      ...
      normalColors = [
        `rgb(200,${(1 - val / 45) * 255 + 50 + offSet[size]}, 255)`,
        `rgb(250,200, ${(1 - val / 80) * 255 + offSet[size]})`,
        `rgb(${(1 - val / 80) * 255 + offSet[size]},200,250)`,
      ],
    } = this.props;
    ...
    let color =
      extraClassName === "-changing"
        ? changingColors[colorSetIndex]
        : normalColors[colorSetIndex];
    if (!finished && isPivot) color = "rgb(240, 190, 149)";
    ...
    return (
      <>
        <div
          ...
          style={{ height: `${val * heights[size]}px`, background: color }}
        >

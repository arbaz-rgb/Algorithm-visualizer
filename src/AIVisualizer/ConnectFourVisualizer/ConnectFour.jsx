    ...
    <div className={props.id === 0 ? "virtual-piece" : "piece"}>
      <div
        className={
          (props.id === 0 ? "v" : "") +
          `${props.val}` +
          (props.finished && props.val !== null ? "finished" : "")
        }
      ></div>
    </div>
  );
...
/**
 * definition of ConnectFour className
 */
export default class ConnectFour extends Component {
  ...
  setDepth(d) {
    this.setState({
      depth: d,
      minimaxAgent: new MinimaxAgent(d, this.state.humanPlayer),
    });
  }
  ...
  AITakeMove() {
    if (
      checkWinner(this.state.board) === null &&
    ...
    let buttons = (
      <>
        <button
          ...
          onClick={() =>
            this.setStartingPlayer(this.state.aiPlayer === 1 ? "ai" : "human")
          }
          type="button"
          className="btn btn-outline-dark"
        >
          <p style={{ "margin-top": "-5px" }}>{`offensive: ${
            this.state.aiPlayer === 1 ? "you" : "ai"
          }`}</p>
        </button>
          ...
          <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
            <li>
              <button
                ...
                style={{ height: "30px" }}
                onClick={() => this.setDepth(2)}
              >
                <p style={{ "margin-top": "-5px" }}>{`Depth: 2`}</p>
              </button>
              <button
                type="button"
                className="btn btn-light navbtn"
                style={{ height: "30px" }}
                onClick={() => this.setDepth(4)}
              >
                <p style={{ "margin-top": "-5px" }}>{`Depth: 4`}</p>
              </button>
              <button
                type="button"
                className="btn btn-light navbtn"
                style={{ height: "30px" }}
                onClick={() => this.setDepth(6)}
              >
                <p style={{ "margin-top": "-5px" }}>{`Depth: 6`}</p>
              </button>
            </li>
          </div>
        </div>
    ...
    if (this.state.winner) {
      ...
      return (
        <div>
          ...
          <h1 style={{ position: "relative", marginTop: "-170px" }}>
            {this.state.winner === "tie"
              ? "Tie"
              : `Winner: ${
                  this.state.winner === this.state.humanPiece ? "You" : "AI"
                }`}
          </h1>
        </div>
      );
      ...
      <div>
        <div className="game">
          ...
          <img
            ...
            height="100px"
            width="100px"
            style={{ marginLeft: "10px", position: "absolute", zIndex: 0 }}
            alt
          ></img>
          ;
          <div>
            <h5
              className="connectFourDes"
              style={{
                position: "absolute",
                marginTop: "160px",
                marginLeft: "30px",
